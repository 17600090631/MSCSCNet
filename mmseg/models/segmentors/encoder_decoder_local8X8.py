...
class EncoderDecoder_LOCAL8x8(BaseSegmentor):
    ...
    def __init__(self,
        ...
        self.global_bacself.global_cfg = global_cfg
    ...
    def _init_fuse_head(self, fuse_head: ConfigType) -> None:
    ...
    def _fuse_head_forward_train(self, inputs: List[Tensor],
                                       data_samples: SampleList) -> dict:
        """Run forward function and calculate loss for decode head in
        training."""
        losses = dict()
        loss_fuse = self.fuse_head.loss(inputs, data_samples, self.train_cfg)

        losses.update(add_prefix(loss_fuse, 'fuse_edge'))
        return losses

    def _fuse_head_forward_test(self, inputs: Tensor,
                        batch_img_metas: List[dict]) -> Tensor:
        """Run forward function and calculate loss for fuse head in
                inference."""
        fuse_logits, _ = self.fuse_head.predict(inputs, batch_img_metas, self.test_cfg)
        return fuse_logits
    ...
    def loss(self, inputs: Tensor, data_samples: SampleList) -> dict:
        ...
        loss_fuse = self._fuse_head_forward_train(x, data_samples)
    ...
    def slide_inference(self, inputs: Tensor,
        ...
        self.global_model.eval()
        with torch.no_grad():
            _, global_features = self.global_model.encode_decode(inputs).detach()
        ...
        local_feature = preds / count_mat

        fuse_feature = torch.cat((F.interpolate(global_features, size=(h_img, w_img), mode='bilinear'), local_feature),dim=1)
        seg_logits = self._fuse_head_forward_test(fuse_feature, batch_img_metas)
