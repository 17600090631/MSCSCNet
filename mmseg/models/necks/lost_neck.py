...
class LostNeck(BaseModule):
    ...
    def __init__(self,
        ...
        self.f23_convs = nn.ModuleList()

        self.fuse0 = SFTLayer(out_channels)
        self.fuse1 = SFTLayer(out_channels)
        ...
        for i in range(self.start_level, self.backbone_end_level - 1):
            f23_conv = ConvModule(
                ...
                act_cfg=act_cfg,
                inplace=False)
            self.f23_convs.append(f23_conv)
    ...
    def forward(self, inputs):
        ...
        feature_31 = laterals[1] + resize(laterals[3], size=prev_shape, **self.upsample_cfg)
        features.append(feature_31)



        prev_shape = laterals[0].shape[2:]
        feature_20 = laterals[0] + resize(laterals[2], size=prev_shape, **self.upsample_cfg)
        features.append(feature_20)

        feature_30 = laterals[0] + resize(laterals[3], size=prev_shape, **self.upsample_cfg)
        features.append(feature_30)

        aux_out = [self.f23_convs[i](features[i]) for i in range(len(features))]
        ...
        # merge features23
        aux_feature0 = aux_out[-1] + aux_out[-2]
        outs[0] = self.fuse0(outs[0], aux_feature0)

        aux_feature1 = aux_out[0]
        outs[1] = self.fuse1(outs[1], aux_feature1)
